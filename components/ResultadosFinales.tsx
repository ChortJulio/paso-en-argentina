...
export function ResultadosFinales({ participantes, onReiniciar }: ResultadosFinalesProps) {
  const participantesOrdenados = [...participantes].sort((a, b) => b.puntaje - a.puntaje)
  ...
  const getIcon = (posicion: number) => {
    switch (posicion) {
      ...
      default:
        return <span className="w-6 h-6 flex items-center justify-center text-sky-700 font-bold">{posicion + 1}</span>
    }
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-400 via-sky-500 to-sky-600 flex items-center justify-center p-4">
      <Card className="max-w-4xl w-full bg-white/95 backdrop-blur-sm shadow-2xl border-0">
        <CardHeader className="text-center pb-6 bg-gradient-to-r from-sky-600 to-sky-700 text-white rounded-t-lg">
          ...
          <CardTitle className="text-3xl md:text-4xl mb-2">¡Resultados Finales!</CardTitle>
          <p className="text-lg opacity-90">¿Pasó en Argentina? - Partida completada</p>
        </CardHeader>
        ...
        <CardContent className="p-6">
          <div className="space-y-4 mb-6">
            ...
            {participantesOrdenados.map((participante, index) => (
              <div
                key={participante.id}
                className={`grid grid-cols-4 gap-4 p-4 rounded-lg border-2 transition-all duration-200 ${getRowStyle(index)}`}
              >
                <div className="flex items-center gap-2">{getIcon(index)}</div>
                <div className="flex items-center">
                  <span className="font-semibold text-gray-800 truncate">{participante.nombre}</span>
                </div>
                ...
                <div className="text-center">
                  <Badge className="bg-sky-600 text-white font-bold">{participante.puntaje} pts</Badge>
                </div>
              </div>
            ))}
          </div>
